; partners' netid: zijunz6, diqingz2
...
; Use the property for 2's complement numbers that if it starts with a 1 it's negative and 0 otherwise, take the MSB and leftshift each time to copy the four most significant bits to the four LSB of another register. Whatever stores in R0 will be printed to the terminal with the code "OUT". 

; R0 = character to print
; R1 = pointer/address of histogram entries
; R2 = counter for histogram rows
; R3 = content of the current histogram row
; R4 = leftshift counter for 1 binary number
; R5 = leftshift counter for 1 hex number
; R6 = temporary variable

LD R2,TWENTY_EIGHT 	; load the number 28 into R2

ONE_ROW
	ADD R2,R2,#-1		; decrement row counter R2 by 1
	BRnz DONE		; 27 rows of histogram printed
	NOT R6,R2		; invert R2 and save it to R6, for subtraction
	LD R3,TWENTY_EIGHT	; temporarily load decima; 28 into R3
	ADD R6,R6,R3		; R6: difference between R2 and 27. Plus 28 because subtraction is flipped bits plus 1. 
	LD R1,HIST_ADDR		; load histogram starting address into R1
	ADD R1,R1,R6		; R1 now is the correct histogram pointer
	LDR R3,R1,#0		; load the content in R1 into R3
	LD R0,AT_VAL		; temporarily save '@' in R0
	ADD R0,R0,R6		; R0 now is the ascii char for current row
	OUT			; print the character for current row
	LD R0,SPACE		; R0 now is the space character
	OUT			; print a space
	LD R5,FIVE		; initialize hex counter R5 to 5 (decrement right after)

ONE_HEX
	ADD R5,R5,#-1 		; decrement R5 by 1, while also load to cc
	BRnz NEW_ROW		; next row when the four hex is printed
	AND R6,R6,#0		; clear R6
	LD R4,FOUR		; initialize R4 to 4

ONE_BIN
	ADD R3,R3,#0		; load R3 into cc
	BRzp #1			; if MSB is currently 0, skip adding 1 to R0
	ADD R6,R6,#1		; add 1 to R0 if MSB is currently 1
	ADD R4,R4,#-1		; decrement binary counter by 1
	BRnz #1			; if it's the last one don't left shift
	ADD R6,R6,R6		; leftshift R6, the digit to be printed
	ADD R3,R3,R3		; leftshift R3, the content of current row
	ADD R4,R4,#0		; load R4 into cc
	BRp ONE_BIN		; branch to next binary bit
	; otherwise 4 bits are processed. print the hex
	ADD R0,R6,#-9		; temporarily use R0 to see whether to print number or letter
	BRnz #1			; if it's a number skip next line for printing letters
	ADD R6,R6,#7		; skip the clon to at part
	LD R0,ZERO_VAL		; load decimal 48 into R0
	ADD R0,R0,R6		; set R0 to the character to be printed
	OUT			; print R0 out
	BR ONE_HEX		; go to the next hex

NEW_ROW
	LD R0,NEW_LINE		; load the newline char value into R0
	OUT			; print a new line
	BR ONE_ROW		; branch to process a new row
...
AT_VAL		.FILL x0040	; the hexidecimal value of '@'
ZERO_VAL	.FILL x0030	; the hex value of '0'
NEW_LINE	.FILL x000A	; the newline character
SPACE		.FILL x0020	; the space character
TWENTY_EIGHT	.FILL x001C	; constant decimal 28
FIVE		.FILL x0005	; constant decimal 5
FOUR		.FILL x0004	; constant decimal 4
...
; partner netid: staceyl2, diqingz2
